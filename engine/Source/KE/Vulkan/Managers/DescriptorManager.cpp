...
DescriptorManager::DescriptorManager () : IVKManager()
    {
    LogDebug ( "created" );
    }
...
VkDescriptorSetLayout DescriptorManager::GetDescriptorSetLayout ( const std::string & LayoutName ) const
    {
    auto it = m_LayoutCache.find ( LayoutName );
    if (it != m_LayoutCache.end ())
        {
...
bool DescriptorManager::HasDescriptorSetLayout ( const std::string & LayoutName ) const
    {
...
VkDescriptorPool DescriptorManager::CreateDescriptorPool (
...
VkDescriptorPool DescriptorManager::GetDescriptorPool ( const std::string & PoolName ) const
    {
    auto it = m_PoolCache.find ( PoolName );
    if (it != m_PoolCache.end ())
        {
...
bool DescriptorManager::HasDescriptorPool ( const std::string & PoolName ) const
    {
...
std::vector<VkDescriptorSet> DescriptorManager::AllocateDescriptorSets (
    const std::string & PoolName,
...
void DescriptorManager::FreeDescriptorSet ( const std::string & PoolName, VkDescriptorSet DescriptorSet )
    {
    FreeDescriptorSets ( PoolName, { DescriptorSet } );
    }

void DescriptorManager::FreeDescriptorSets ( const std::string & PoolName, const std::vector<VkDescriptorSet> & DescriptorSets )
    {
...
void DescriptorManager::UpdateDescriptorSets ( const std::vector<VkWriteDescriptorSet> & Writes )
    {
...
bool DescriptorManager::CreateDefaultLayouts ()
    {
...
bool DescriptorManager::CreateDefaultPools ()
    {
...
bool DescriptorManager::ResetDescriptorPool ( const std::string & PoolName )
    {
    auto poolIt = m_PoolCache.find ( PoolName );
    if (poolIt == m_PoolCache.end ())
        {
...
bool DescriptorManager::ResetDescriptorPool ( VkDescriptorPool Pool )
    {
    if (Pool == VK_NULL_HANDLE)
        {
...
VkDescriptorSetLayout DescriptorManager::CreateDescriptorSetLayoutInternal (
    const std::vector<VkDescriptorSetLayoutBinding> & Bindings )
    {
    auto * deviceMgr = static_cast< DeviceManager * >( m_info->Managers.DeviceManager.get () );
    VkDevice device = deviceMgr->GetDevice ();
...
VkDescriptorPool DescriptorManager::CreateDescriptorPoolInternal (
    uint32_t MaxSets,
    const std::vector<VkDescriptorPoolSize> & PoolSizes,
    VkDescriptorPoolCreateFlags Flags )
    {
...
const std::string & DescriptorManager::GetManagerName () const
    {
