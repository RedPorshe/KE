...
bool CCollisionSystem::bIsGlobalShuttingDown = false;  // Инициализация новой переменной
static bool bIsAlreadyShuttingDown = false;  // Локальный флаг для Shutdown
...
CCollisionSystem::~CCollisionSystem ()
    ...
    if (!m_CollisionComponents.empty ())
        ...
        // Если это синглтон, сбрасываем указатель
...
CCollisionSystem & CCollisionSystem::Get ()
    {
        // Не создаём новый экземпляр, если система выключается
    if (!s_Instance && !s_IsInitialized && !bIsGlobalShuttingDown && !bIsAlreadyShuttingDown)
        {
...
void CCollisionSystem::Update ( float deltaTime )
    {
        // Не обновляем во время выключения
    if (bIsGlobalShuttingDown || bIsAlreadyShuttingDown)
        return;
